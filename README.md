...
- 정규화(regularization)를 왜 해야할까요? 정규화의 방법은 무엇이 있나요?
  - 목적: 오버피팅 방지
  - L1(LASSO): <img src="https://latex.codecogs.com/svg.latex?\sum&space;L(Y,Xw)+\lambda&space;||w||_{1}"/>   
  - L2(Rigde): <img src="https://latex.codecogs.com/svg.latex?\sum&space;L(Y,Xw)+\lambda&space;||w||_{2}^{2}"/>   
  - Elastic Net: <img src="https://latex.codecogs.com/svg.latex?\sum&space;L(Y,Xw)+\lambda(\alpha||w||_{1}+(1-\alpha)||w||_{2}^2)"/>   
